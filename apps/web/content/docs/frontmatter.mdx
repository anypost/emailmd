...
Set `lang` (a language tag like `en`, `de`, or `pt-BR`) and `dir` (`ltr`, `rtl`, or `auto`) to emit the matching attributes on the output document's `<html>` tag, so screen readers pick the right voice and right-to-left scripts lay out correctly:
