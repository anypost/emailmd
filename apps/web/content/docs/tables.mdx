...
The delimiter row is still required (that's what makes it a table in GFM), and column alignment set there still applies. This is a common markdown convention: in editors that don't support it, the table simply shows an empty header row instead of breaking.
