...
If no file is given, the CLI reads from stdin. The `lint` command checks the email instead of rendering it; see [Linting](#linting) below. The `mcp` command runs the [MCP server](/docs/mcp) for AI clients; see [MCP Server](#mcp-server) below.
...
| `--escape-html`    | Escape raw HTML for untrusted input. See [Raw HTML](/docs/api-reference#raw-html). |
...
Escape raw HTML when the source is untrusted, so raw tags render as text instead of markup. This also drops unsafe `{attr=…}` attributes and escapes HTML inside template tags. See [Raw HTML](/docs/api-reference#raw-html):
...
`emailmd lint` checks the email for deliverability, accessibility, and readability problems instead of rendering it: missing alt text, `http://` links, Gmail's 102KB clip limit, generic link text, spam-trigger phrases, and more. See [Linting](/docs/linting) for the full rule list.
...
`emailmd mcp` runs the [MCP server](/docs/mcp) over stdio, giving a local AI client tools to render, lint, and look up docs. `--partials` preloads [partials](/docs/partials) for every render and lint call:
