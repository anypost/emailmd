...
Only keys explicitly passed on the include line are substituted. Every other `{{token}}` passes through untouched for your app's template layer (Handlebars, Liquid, …) to resolve after render, following the same [pass-through contract](/docs/api-reference#renderoptions) as the rest of the pipeline. In the footer example above, `{{unsubscribe_url}}` survives into the output HTML exactly as written.
...
- **[Wrappers](/docs/wrappers)** replace the whole document chrome in code, the right tool for layout-level control rather than content reuse.
