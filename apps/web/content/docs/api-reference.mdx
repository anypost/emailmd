...
| `css`             | `string`                                      | Custom CSS emitted as one extra `<mj-style>` in the head, after the built-in styles so it can override them. Reaches renderer classes like `.emd-btn`. Inline styles still win — use `!important`; and as an embedded `<style>`, not every client honours it. |
