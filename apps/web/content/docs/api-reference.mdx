...
| `html`            | `boolean`                                     | Allow raw HTML in the Markdown source. Default `true` (raw tags pass through). Set `false` for untrusted content to escape raw tags (`<script>` → text) while keeping every Markdown feature. See [Untrusted content](#untrusted-content). |
...
By default (`html: true`) raw HTML in the Markdown source passes through verbatim —
`<span style="…">`, `<div>`, and inline formatting tags all render as written. This
is convenient for templates you control.

When the Markdown comes from somewhere you do **not** control — user-generated
campaign copy, a public submission, anything an end user can type — pass
`html: false`:

```typescript
const { html } = await render(userMarkdown, { html: false });
```

With `html: false`, raw tags are escaped to text (`<script>alert(1)</script>` becomes
the literal characters, not an element), which closes off HTML and script injection
into the rendered email. Everything else is unaffected: headings, emphasis, links,
lists, tables, images, emoji, directives (`::: callout`), and buttons all keep
working, and `javascript:`/`data:` URLs in Markdown links are blocked whether or not
this option is set.
