...
| `css`             | `string`                                      | Custom CSS emitted as one extra `<mj-style>` in the head, after the built-in styles so it can override them. Reaches renderer classes like `.emd-btn`. Inline styles still win, so use `!important`, and as an embedded `<style>` not every client honours it. Trusted developer input only, not end-user-controlled strings. |
