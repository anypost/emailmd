---
title: React Builder
description: Drop the full emailmd builder into your React app, or use the hooks and preview components on their own.
---

`@emailmd/react` packages the entire [emailmd.dev builder](/builder) as a drop-in component, plus the lower-level pieces (a render hook and a sandboxed preview) for building your own UI.
...
The full [emailmd.dev builder](/builder), as a drop-in component: CodeMirror markdown editor with a formatting toolbar, visual theme editor, emoji and snippet pickers, live preview with device switching and a dark-mode toggle (enabled when the email uses `theme: auto`), HTML source (pretty/minified with a Gmail-clip meter), plain-text view, render warnings, autosave, download, and optional share links. The builder page on this site is exactly this component.
...
| `lint` | `boolean` | `false` | Run [`lint()`](/docs/linting) on each render and surface findings in the warnings banner |
...
builderRef.current?.setMarkdown(generated);
```
...
Options extend [`RenderOptions`](/docs/api-reference#renderoptions), so `theme`, `fonts`, `minify`, `strings`, and the rest all pass through, plus:

| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `debounceMs` | `number` | `150` | Wait after the last keystroke before re-rendering. `0` renders immediately. |
| `lint` | `boolean` | `false` | Run [`lint()`](/docs/linting) alongside each render; findings land in `lintFindings`. |
...
| `emulateColorScheme` | `'light' \| 'dark'` | — | Pin the preview to one of the email's [color-scheme variants](/docs/theme#automatic-dark-mode) regardless of the viewer's OS; unset follows the OS |
