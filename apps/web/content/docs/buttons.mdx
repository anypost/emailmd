...
Buttons take their font size from the theme's `fontSize`, so they scale with the body text. A smaller `fontSize` gives a smaller button.

## Styling

Every button carries the class `emd-btn`, so you can reach properties that have no dedicated option (weight, padding, …) through the [`css`](/docs/api-reference#renderoptions) render option:

```typescript
await render(md, {
  css: '.emd-btn a { font-weight: 400 !important }',
});
```

The `!important` is needed because the button's own styles are inline. Because this relies on an embedded `<style>`, treat it as progressive enhancement: clients that don't honour those keep the built-in styling.

The `css` string is emitted verbatim, so pass it only trusted developer input, never end-user-controlled strings.
...
To change the default message globally instead of per button, pass `strings.buttonFallback` to `render()`. It supports `{text}` and `{url}` placeholders:

```typescript
await render(md, {
  strings: {
    buttonFallback: "Si vous avez des difficultés avec le bouton « {text} », copiez cette URL : {url}",
  },
});
```

A per-button `fallback="..."` attribute still wins over the global string. See [`RenderStrings`](/docs/api-reference#renderstrings).
